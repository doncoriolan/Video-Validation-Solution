...
11. Image can now be tested using `docker run -p80:80 -p443:443 --volume=persistent_data:/opt/vvs -it <Container Image>`, an `--rm` can be used to automatically destroy the container after it's shutdown to limit changes only to persistent storage
